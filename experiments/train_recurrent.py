...
parser.add_argument('--n_layers', default=8, type=int, help='default number of iterations')
...
# process args
args.recurrent = True # set flag to let utils module know that we are using a recurrent model
...
if __name__ == "__main__":
    ...
    n_iter_schedule = lambda nvars: nvars # FIXME: dummy schedule for now (maybe int(log nvars) or something; or organize curriculum by nsteps)
    for curriculum_nvars in range(min_nvars, max_nvars+1):
        # NOTE: alternatively, arrange curriculum by number of steps rather than number of variables
        # FIXME: this is a rough test just to see if it works
        ...
        # update default_n_iters in RecurrentTransformerLM model
        net.model.default_n_iters = n_iter_schedule(curriculum_nvars)
